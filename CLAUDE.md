...
**Bot players:** the host adds/removes bots in the waiting room (`room:addBot {speed}` / `room:removeBot {playerId}`). There is only one bot *difficulty* — every bot is created with `botLevel: 'hard'` (the old easy/hard picker was removed from the UI) — but the host does pick a *play speed* per bot (`botSpeed`: `'fast' | 'normal' | 'slow'`, default `'slow'`, invalid values fall back to the default; the picker is the `<select>` next to the ➕ 電腦 button in `Room.jsx`). The speed profiles live in `BotDriver.BOT_SPEEDS` (`delayMs` = fake thinking pause before the first tile, `min/maxStepMs` = per-tile step range; `normal` reproduces the old hard-coded 1200ms/150–600ms behavior). `BotDriver.stepMsFor` keeps the per-tile step inside half the turn time *and* caps thinking + placement at 70% of the turn, so a slow bot with many tiles can't run into the 15s-minimum turn timer. A bot is a socketless entry in `room.players` (`isBot: true`, `botLevel`, `botSpeed`, `socketId: null`, always `connected`/`ready` — `toPlayer` no-ops safely for it). `room:start` builds a `BotDriver` per room when bots are present, passing `botSpeedOf`; `onGameOver` disposes it and keeps bots ready for a rematch. Room-lifecycle rules that must stay bot-aware: host succession skips bots (a bot can never be host; an all-bot room is destroyed), and `scheduleDestroyIfAbandoned` ignores bots when checking for connected players (they are always "connected"). The client "AI 代出牌" button (`TurnControls.jsx`) runs the same solver locally: `game:reset` → `solve()` → stepped `game:layout` (via `layoutSteps`) → `game:endTurn`, falling back to `game:draw` — the server still fully validates, so a buggy client solve can't cheat. Its pacing is deliberately *randomized and slow-leaning* (`humanStepDelays`) so opponents can't spot the AI by its rhythm: a random 1.2–3.2s think before the first tile, jittered per-tile steps (base ≤900ms × 0.7–1.6, plus a 15% chance of an extra 0.4–1.0s hesitation), a short pause before 出牌, and a 1.5–3.5s pause before drawing when there's no play; total placement is scaled to fit 65% of the turn. The button is a hidden feature: tapping the room name 5 times within 2s (`useSecretTaps.js`, wired to `.room-tag` in `PlayerBar.jsx` and the `<h1>` in `Room.jsx`) toggles `aiUnlocked` (persisted in localStorage as `rummy-ai-unlocked`). Next to it is an "自動" checkbox (`aiAuto`, not persisted) that auto-plays every turn via a `useEffect` on `myTurn`; hiding the feature also switches auto mode off.

**Client store contract:** `store.js`'s `bindSocket()` is the single source of truth for how server events map to UI state — e.g. `game:state` triggers the "your turn" banner+beep only when `current` changes to the local player; `game:drew` drives both a temporary rack highlight and a center-screen overlay animation; `chat:message` appends to `chat` and, when the chat panel is collapsed (`chatOpen: false`) and the message is someone else's non-system message, also pushes a `chatPops` bubble (`pushChatPop`, max 3, self-evicting after 3.5s, rendered globally in `App.jsx`, click opens the chat) and plays `sounds.chat()`. When adding a new server-to-client event, add the listener there rather than in individual components.
